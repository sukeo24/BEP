        ...
        if idx <= 3:
            with tab:
                ...
                with left_col:
                    ...
                    st.markdown(f"### 💰 初期費用内訳", unsafe_allow_html=True)
                    ...
                    st.markdown(f"### 📈 シミュレーション設定", unsafe_allow_html=True)
                    sales = st.number_input("月間売上（税込）[万円]", value=500, step=10, key=f"sales_{idx}")
                    contribution_margin = st.slider("貢献利益率（粗利率）", min_value=0.4, max_value=0.8, value=0.64, step=0.01, key=f"margin_{idx}")
            ...
            with tab:
                ...
                for i, result in enumerate(results):
                    fig.add_trace(go.Scatter(
                        ...
                        mode="lines", name=f'{result["予測"]}：予想売上',
                        line=dict(color=colors[i], width=2)
                    ))
                    fig.add_trace(go.Scatter(
                        x=result["x"], y=result["bep_line"],
                        mode="lines", name=f'{result["予測"]}：損益分岐点売上',
