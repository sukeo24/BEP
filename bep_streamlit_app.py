...
with left_col:
    ...
    area = st.number_input("坪数 [坪]", value=20)
    tsubo_unit_price = st.number_input("坪単価 [万円]", value=5.0)

    st.subheader("💰 初期費用内訳")
    key_money = st.number_input("礼金 [万円]", value=0)
    deposit = st.number_input("敷金 [万円]", value=0)
    guarantee_money = st.number_input("保証金 [万円]", value=0)
    agency_fee = st.number_input("仲介手数料 [万円]", value=0)
    interior_cost = st.number_input("内装工事費 [万円]", value=0)
    others = st.number_input("その他費用 [万円]", value=0)
